...
def save_data(students, courses, mark_manager):
    """
    1. Dump objects to pickle files.
    2. Compress pickle files into students.dat.
    3. Remove temporary pickle files.
    """
    try:
        ...
        with zipfile.ZipFile('students.dat', 'w', compression_method) as zf:
...
def load_data():
    """
    1. Check if students.dat exists.
    2. Decompress files.
    3. Load objects from pickle files.
    4. Clean up temp files.
    5. Return loaded data or empty structures.
    """
    students = []
    courses = []
...
def main(stdscr):
    ...
    while True:
        ...
        if students:
            stdscr.addstr(0, 0, f"[Loaded {len(students)} students]", curses.A_DIM)
            ...
            if current_row == 0:
                ...
                if selected_course:
                    ...
                    for s in students:
                        ...
                        current_marks = mark_manager._MarkManager__marks.get(cid_in, {}) 
                        prev_mark = current_marks.get(s.get_id(), "")
                        prompt = f"Mark for {s.get_name()} (Cur: {prev_mark}): "
                ...
                stdscr.clear()
                ui_out.print_message(stdscr, 5, 10, "Compressing data to students.dat...")
                stdscr.refresh()
